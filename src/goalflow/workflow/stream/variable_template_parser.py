...
def extract_selectors_from_template(template: str, /) -> Sequence[NodeVarConfig]:
    parts = SELECTOR_PATTERN.split(template)
    selectors = []
    for part in filter(lambda x: x, parts):
        if "." in part and part[0] == "#" and part[-1] == "#":
            selectors.append(NodeVarConfig(variable=f"{part}", value_selector=part[1:-1].split(".")))
    return selectors
...
class VariableTemplateParser:
    ...
    def extract_variable_selectors(self) -> list[NodeVarConfig]:
        """
        ...
        for variable_key in self.variable_keys:
            ...
            variable_selectors.append(NodeVarConfig(variable=variable_key, value_selector=split_result))
